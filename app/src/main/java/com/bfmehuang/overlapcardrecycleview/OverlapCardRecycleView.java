...
public class OverlapCardRecycleView extends RecyclerView {

    private OverlapCardRemovedListener mRemovedListener;
    private float mTopViewX;
    private float mTopViewY;
    private float mTopViewOffsetX = 0;
    private float mTopViewOffsetY = 0;
    private float mBorder = dip2px(120);//临界线，临界点

    private float dip2px(float dip) {
        return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,dip,getResources().getDisplayMetrics());
    }
    public void setRemovedListener(OverlapCardRemovedListener listener) {
    ...
    public OverlapCardRecycleView(Context context) {
        super(context);
        init();
    }


    public OverlapCardRecycleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public OverlapCardRecycleView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        if (getChildCount() == 0) {//所有布局都移除了之后
            return super.onTouchEvent(e);
        }
        View topView = getChildAt(getChildCount() - 1);//最上面的那个view
        float touchX = e.getX();//手指按下时候的x点
        float touchY = e.getY();
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN://按下的时候记录
     ...
     * @return
     */
    private View getMirrorView(View view) {
        ...
        ((BaseAdapter) getAdapter()).delTopItem();
